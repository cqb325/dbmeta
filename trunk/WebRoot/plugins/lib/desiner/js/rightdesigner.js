...
SQL.Row.prototype._build = function() {
	...
	this.dom.selected = OZ.DOM.elm("input", {className:"selected",type:"checkbox"});
...
//	OZ.Event.add(this.dom.container, "click",this.bind(this.click));
	OZ.Event.add(this.dom.selected, "click",this.bind(this.checkclick));
}
SQL.Row.prototype.checkclick = function(){
	this.data["def"] = this.dom.selected.checked;
	var checked = this.data["def"];
	for(var i in this.relations){
		var r = this.relations[i];
		if(r.row1 == this){
			continue;
		}
		var rtable = r.row1.owner;
		for(var j in rtable.rows){
			var rrow = rtable.rows[j];
			rrow.data["def"] = checked;
			rrow.redraw();
		}
	}
	
	var ownerrs = this.owner.getRelations();
	var ownerrows = this.owner.rows;
	var parentcheck = false;
	for(var i in ownerrows){
		var def = ownerrows[i].data["def"];
		if(def){
			parentcheck = true;
			break;
		}
	}
	for(var i in ownerrs){
		if (ownerrs[i].row2.owner != this.owner) {
			ownerrs[i].row2.data["def"] = parentcheck;
			ownerrs[i].row2.redraw();
		}
	}
}
...
SQL.Row.prototype.redraw = function() {
	...
	this.dom.container.style.display = this.data.id ? "": "none";
//	this.dom.container.title = this.data.comment;
	if(this.data["def"]){
		this.dom.selected.checked = true;
	}else{
		this.dom.selected.checked = false;
	}
...
SQL.Row.prototype.fromXML = function(node) {
	...
	var restype = node.getAttribute("restype");
...
SQL.Table.prototype.getTableData = function(){
	var data = {};
...
SQL.Designer.prototype.getTableData = function(){
	var data = [];
	for (var i=0;i<this.tables.length;i++) {
		data.push(this.tables[i].getTableData());
	}
